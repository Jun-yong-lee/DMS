...
def receive_CAN_C(d_name, save_flag, DATASET_PATH, P_db, C_db, can_bus, print_status, stop_event):
    ...
    C_msg_list = ['HEV_PC1', 'HEV_PC2', 'HEV_PC4', 'HEV_PC5','HEV_PC6', 'HEV_PC12', 'SAS11', 'ESP12', 'WHL_SPD11', \
                  'CGW1', 'CLU12', 'CLU15', 'DATC3', 'CGW4', 'TCS15', 'BCW11']
    ...
    C_signal_names = ['CF_Ems_EngStat', 'CR_Brk_StkDep_Pc', 'CR_Ems_AccPedDep_Pc', 'CR_Ems_EngSpd_rpm', 'CR_Ems_FueCon_uL', 'CR_Ems_VehSpd_Kmh', \
                    ...
                    'CF_Clu_InhibitP', 'CF_Clu_InhibitR', 'CF_Clu_VehicleSpeed', 'CF_Clu_Odometer', 'CF_Gway_TSigLHSw', 'CF_Gway_TSigRHSw', \
    ...
    can_monitoring = {'ESP12': -1, 'SAS11': -1, 'WHL_SPD11': -1, 'IND_LEFT': -1, 'IND_RIGHT': -1}
                ...
                if can_msg.arbitration_id == msg.frame_id:
                    ...
                    if print_status:
                        if msg.name == 'ESP12':
                            ...
                            can_monitoring['IND_LEFT'] = can_dict['CF_Gway_TSigLHSw']
                            can_monitoring['IND_RIGHT'] = can_dict['CF_Gway_TSigRHSw']
            if print_status:
                ...
                print("[INFO] TIME[{}] IND_LEFT[{}] IND_RIGHT[{}] SAS_Angle[{:7.2f}]".format(record_time, can_monitoring['IND_LEFT'], can_monitoring['IND_RIGHT'], can_monitoring['SAS11']), end='\r')
...
def receive_CAN_M(d_name, save_flag, DATASET_PATH, M_db, can_bus, print_status, stop_event):
    print(f"[INFO] PID[{os.getpid()}] '{d_name}' process is started.")
    ...
    timestamp_cols = ['timestamp', 'timestamp2']
    df = pd.DataFrame(columns=timestamp_cols)
    can_monitoring = {'IND_LEFT': -1, 'IND_RIGHT': -1}
    ...
    while(True):
        try:
            ...
            for msg in db_msg:
                if can_msg.arbitration_id == msg.frame_id:
                    can_dict = M_db.decode_message(can_msg.arbitration_id, can_msg.data)
                    can_dict = {k: v for k, v in can_dict.items() if k in M_signal_names}
                    can_dict['timestamp'] = can_msg.timestamp
                    can_dict['timestamp2'] = timestamp2

                    if len(df.columns) >= len(M_signal_names) + len(timestamp_cols):
                        if save_flag:
                            if first:
                                df.to_csv(CAN_PATH + f"{start_time}_M.csv", index=False)
                                first = False
                            else:
            ...
            if print_status:
                record_time = str(dt.timedelta(seconds=(st - st_time))).split(".")[0]
                print("[INFO] TIME[{}] IND_LEFT[{}] IND_RIGHT[{}]".format(record_time, can_monitoring['IND_LEFT'], can_monitoring['IND_RIGHT']), end='\r')
            time_total += (time.time() - st)
            cycle += 1
